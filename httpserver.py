...
(and even that is often done indirectly via `nida.web.Application.listen`).

This module also defines the `HTTPRequest` class which is exposed via
`nida.web.RequestHandler.request`.
"""


"""
modify by:shawnsha@tencent.com
date: 2016.8.20
"""
...
from nida.escape import native_str, parse_qs_bytes
from nida import httputil
from nida import iostream
from nida.log import gen_log
from nida.util import netutil
from nida.tcpserver import TCPServer
from nida import context_manager
from nida.util.util import bytes_type
...
class HTTPServer(TCPServer):
    ...
    requested::
        ...
        nida.ioloop.IOLoop.instance().start()
    ...
    initialization methods are defined on `nida.tcpserver.TCPServer`):

    1. `~nida.tcpserver.TCPServer.listen`: simple single-process::

            server = HTTPServer(app)
            server.listen(8888)
            IOLoop.instance().start()

       In many cases, `nida.web.Application.listen` can be used to avoid
       the need to explicitly create the `HTTPServer`.

    2. `~nida.tcpserver.TCPServer.bind`/`~tornado.tcpserver.TCPServer.start`:
    ...
    3. `~nida.tcpserver.TCPServer.add_sockets`: advanced multi-process::

            sockets = nida.netutil.bind_sockets(8888)
            nida.process.fork_processes(0)
            server = HTTPServer(app)
            server.add_sockets(sockets)
            IOLoop.instance().start()

       The `~.TCPServer.add_sockets` interface is more complicated,
       but it can be used with `nida.process.fork_processes` to
       give you more flexibility in when the fork happens.
       `~.TCPServer.add_sockets` can also be used in single-process
       servers if you want to create your listening sockets in some
       way other than `nida.netutil.bind_sockets`.

    """
    def __init__(self, request_callback, no_keep_alive=False, io_loop=None,
                 xheaders=False, protocol=None, **kwargs):
...
class HTTPConnection(object):
    ...
    def __init__(self, stream, address, request_callback, no_keep_alive=False,
        ...
        self._header_callback = context_manager.wrap(self._on_headers)
...
class HTTPRequest(object):
    ...
    def __init__(self, method, uri, version="HTTP/1.0", headers=None,
        ...
        if protocol:
            if protocol == "https":
                app_log.error("Not support https now")
                protocol = "http"
            self.protocol = protocol
